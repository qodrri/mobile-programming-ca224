...
class ApiMomentRepository extends AbsApiMomentRepository {
  ...
  Future<Moment?> create(Moment newData) async {
    try {
      final response = await _dio.post('', data: newData.toDto());
  ...
  Future<List<Moment>> getAll([String keyword = '']) async {
    try {
      final response =
          await _dio.get('/all', queryParameters: {'keyword': keyword});
      if (response.statusCode == 200) {
        return (response.data as List)
            .map((moment) => Moment.fromMap(moment))
            .toList();
      }
    } catch (e) {
  ...
  Future<bool> update(Moment updatedData) async {
    try {
      final response =
          await _dio.put('/${updatedData.id}', data: updatedData.toDto());
      if (response.statusCode == 204) {
        return true;
      }
    } catch (e) {
